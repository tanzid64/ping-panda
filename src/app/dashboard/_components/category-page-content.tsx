...
export const CategoryPageContent: FC<CategoryPageContentProps> = ({
  ...
  // baseUrl/dashboard/category/[name]?page=1&limit=30
  const page = parseInt(serchParams.get("page") || "1", 10);
  const limit = parseInt(serchParams.get("limit") || "30", 10);
  ...
  // Table -> Logic from Tanstack, style from shadcn table
  ...
  const NumericFieldSumCards = () => {
    ...
    return Object.entries(numericFieldSums).map(([field, sum]) => {
      const relevantSum =
        activeTab === "today"
          ? sum.today
          : activeTab === "week"
